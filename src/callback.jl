    ...
    return function callback(state, args...)
        if length(args) > 0
            if verbose ≥ 3 && state.t == 1
                @printf(stdout, "%s    %s    %s    %s    %s   %s    %s\n", lpad("Iteration", 12), lpad("Primal", 10), lpad("Dual gap", 10), lpad("Time (sec)", 10), lpad("#It/sec", 10), lpad("#Atoms", 7), lpad("#LMO", 7))
            end
            active_set = args[1]
            ...
            if verbose ≥ 3 && mod(state.t, callback_interval) == 0
                @printf(stdout, "%s    %.4e    %.4e    %.4e    %.4e   %s    %s\n", lpad(state.v.lmo.data[1], 12), state.primal, state.dual_gap, state.time, state.t/state.time, lpad(length(active_set), 7), lpad(state.v.lmo.data[2], 7))
            end
