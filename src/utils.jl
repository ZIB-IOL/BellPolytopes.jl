...
function alternating_minimisation!(
    ...
    @inbounds while true
        ...
        sc1 = dot(ax[1], lmo.tmp[1])
        sc2 - sc1 > 10Base.rtoldefault(T) || break
    end
    return sc1
end
...
function _normalize!(v::AbstractVector{T}, a::T = one(T)) where {T <: Number}
    ...
    if dot(v, v) == a^2 # to avoid numerical issues for [0, 1]
        return
    else
...
function grassc(d::Int, n::Int)
    v = reshape(parse.(Float64, readlines(joinpath(@__DIR__, "..", "grassc", "grassc.$d.1.$n.txt"))), d, n)'
