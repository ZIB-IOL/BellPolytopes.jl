...
export bell_frank_wolfe, nonlocality_threshold
include("quantum_utils.jl")
export ketbra, qubit_mes, BNV16_vec, HQVNB17_vec, rho_singlet, rho_GHZ, rho_W
include("types.jl")
...
function bell_frank_wolfe(p::Array{T, N};
    ...
    if p ≈ reynolds_permutedims(p)
        ...
        if sym == true
            @warn "Input array seemingly inconsistant with sym=true"
        else
            reynolds = nothing
        end
    end
