...
@testset "Testing Bell-Frank-Wolfe with d=2, N=2, and cor/marg matrices   " begin
    ...
    @test abs(bell_frank_wolfe(correlation_tensor(HQVNB17_vec(3), 2; rho=rho_GHZ(2), marg=true); marg=true, active_set=BellPolytopes.load_active_set(BellPolytopes.ActiveSetStorage(bell_frank_wolfe(correlation_tensor(HQVNB17_vec(3), 2; rho=rho_GHZ(2), marg=true); marg=true, max_iteration=10)[6])))[3] - 0.613691) < 1e-5
    :wqa
