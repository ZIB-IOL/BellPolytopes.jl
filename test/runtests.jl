...
@testset "Testing Bell-Frank-Wolfe with d=2, N=2, and correlation matrices" begin
    ...
    @test abs(bell_frank_wolfe(correlation_matrix(HQVNB17_vec(3)); sym=false, use_array=false)[3] - 0.613691) <
          1e-5
    @test abs(bell_frank_wolfe(correlation_matrix(HQVNB17_vec(3)); sym=false, use_array=true)[3] - 0.613691) <
          1e-5
...
@testset "Testing Bell-Frank-Wolfe with d=2, N=2, and cor/marg matrices   " begin
    ...
    @test abs(
        bell_frank_wolfe(correlation_tensor(HQVNB17_vec(5), 2; rho=rho_GHZ(2), marg=true); marg=true)[3] -
        5.69,
    ) < 1e-1
